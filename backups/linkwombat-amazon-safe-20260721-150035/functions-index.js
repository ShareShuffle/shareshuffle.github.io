...
// LinkWombat 1.0 — friendly short links for classrooms.
...
async function wombatClaimRandom() {
  for (let length=3; length<=5; length++) for (let attempt=0; attempt<24; attempt++) {
    const code=wombatRandomCode(length), ref=db.collection(WOMBAT_COLLECTION).doc(code), snap=await ref.get(); if (!snap.exists) return {path:code,ref};
  }
  throw new Error("NO_CODES");
}
export const wombatCreateLink = onRequest({invoker:"public",timeoutSeconds:15,memory:"256MiB"}, async (req,res)=>{
  setCors(res,"POST, OPTIONS"); if(req.method==="OPTIONS"){res.status(204).send("");return;} if(req.method!=="POST"){res.status(405).json({error:"Method not allowed"});return;}
  const body=typeof req.body==="object"&&req.body?req.body:{}; const url=String(body.url||"").trim(); if(!wombatValidDestination(url)){res.status(400).json({error:"Enter a valid public http or https URL."});return;}
  const handle=wombatNormalizePart(body.handle||""); const alias=wombatNormalizePart(body.alias||"");
  ...
  try{
    let path,ref; if(alias){path=handle?`${handle}/${alias}`:alias;ref=db.collection(WOMBAT_COLLECTION).doc(path);}else({path,ref}=await wombatClaimRandom());
    await db.runTransaction(async tx=>{const snap=await tx.get(ref);if(snap.exists)throw new Error("TAKEN");tx.set(ref,{path,url,handle:handle||null,alias:alias||null,createdAt:FieldValue.serverTimestamp(),updatedAt:FieldValue.serverTimestamp(),clicks:0,active:true});});
    res.set("Cache-Control","no-store");res.status(201).json({ok:true,path,shortUrl:`https://wombat.to/${path}`});
  }catch(error){if(String(error?.message||error).includes("TAKEN")){res.status(409).json({error:"That Wombat address is already taken."});return;}console.error("wombatCreateLink failed",error);res.status(500).json({error:"The wombat could not make that link."});}
});
export const wombatResolve = onRequest({invoker:"public",timeoutSeconds:15,memory:"256MiB"},async(req,res)=>{
  const host=String(req.headers.host||"").toLowerCase(); const raw=decodeURIComponent(String(req.path||"/")).replace(/^\/+|\/+$/g,"");
  if(!raw){res.set("Cache-Control","public,max-age=300");res.redirect(302,"https://linkwombat.com/");return;}
  const path=raw.split("/").map(wombatNormalizePart).filter(Boolean).join("/"); if(!path||path.split("/").some(part=>WOMBAT_RESERVED.has(part))){res.status(404).send("Link not found");return;}
  try{const ref=db.collection(WOMBAT_COLLECTION).doc(path),snap=await ref.get();if(!snap.exists||snap.data()?.active===false){res.status(404).redirect(302,"https://linkwombat.com/404");return;}const data=snap.data()||{};if(!wombatValidDestination(data.url)){res.status(410).send("Link unavailable");return;}ref.set({clicks:FieldValue.increment(1),lastVisitedAt:FieldValue.serverTimestamp()},{merge:true}).catch(()=>{});res.set("Cache-Control","no-store");res.redirect(302,data.url);}catch(error){console.error("wombatResolve failed",error);res.status(500).send("Link unavailable");}
});
